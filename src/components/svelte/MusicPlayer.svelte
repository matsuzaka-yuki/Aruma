<script lang="ts">
	...
	const mode = siteConfig.musicPlayer?.mode ?? "meting";
	...
	const meting_id = siteConfig.musicPlayer?.id ?? "14164869977";
	// Meting API 的服务器，从配置中获取或使用默认值，有的 meting 的 api 源支持更多平台，一般来说，netease=网易云音乐，tencent=QQ 音乐，kugou=酷狗音乐，xiami=虾米音乐，baidu=百度音乐
	const meting_server = siteConfig.musicPlayer?.server ?? "netease";
	// Meting API 的类型，从配置中获取或使用默认值
	const meting_type = siteConfig.musicPlayer?.type ?? "playlist";
...
{#if siteConfig.musicPlayer?.enable}
	...
	>
		<!-- 隐藏状态的小圆球 -->
		<div
			class="orb-player bg-\[var\(--music-player-primary\)\] rounded-full shadow-lg cursor-pointer transition-all duration-500 ease-in-out flex items-center justify-center hover:scale-110 active:scale-95"
		...
		<!-- 收缩状态的迷你播放器（封面圆形） -->
		<div
			class="mini-player bg-\[var\(--music-player-bg\)\] shadow-xl rounded-2xl p-3 transition-all duration-500 ease-in-out"
		...
		>
			<div class="flex items-center gap-3">
				<!-- 封面区域：点击控制播放/暂停 -->
				<div
					class="cover-container relative w-12 h-12 rounded-full overflow-hidden cursor-pointer mp-bg-btn"
					on:click={togglePlay}
					on:keydown={(e) => {
						if (e.key === "Enter" || e.key === " ") {
							e.preventDefault();
				...
				>
					<img
						...
						class:pointer-events-none={isCoverLoading ||
							isImageError}
				...
				>
					<div class="text-sm font-medium mp-text-primary truncate">
						{currentSong.title}
					</div>
					<div class="text-xs mp-text-secondary truncate">
						{currentSong.artist}
					</div>
				</div>
		...
		<!-- 展开状态的完整播放器（封面圆形） -->
		<div
			class="expanded-player bg-\[var\(--music-player-bg\)\] shadow-xl rounded-2xl p-4 transition-all duration-500 ease-in-out"
		...
		>
			<div class="flex items-center gap-4 mb-4">
				<div
					class="cover-container relative w-16 h-16 rounded-full overflow-hidden shrink-0 mp-bg-btn"
				>
					<img
						src={getAssetPath(currentSong.cover)}
						alt={i18n(Key.musicPlayerCover)}
				...
				<div class="flex-1 min-w-0">
					<div
						class="song-title text-lg font-bold mp-text-primary truncate mb-1"
					>
						{currentSong.title}
					</div>
					<div class="song-artist text-sm mp-text-secondary truncate">
						{currentSong.artist}
					</div>
				...
				<div class="flex items-center gap-1">
					...
					>
						<Icon
							...
							class="text-lg"
							style="color: {showPlaylist
								? 'var(--music-player-primary)'
								: ''}"
						/>
					</button>
				</div>
			...
			<div class="progress-section mb-4">
				<div
					class="progress-bar flex-1 h-2 mp-bg-btn rounded-full cursor-pointer"
				...
				>
					<div
						class="h-full bg-\[var\(--music-player-primary\)\] rounded-full transition-all duration-100"
			...
			<div class="bottom-controls flex items-center gap-2">
				...
				<div
					class="flex-1 h-2 mp-bg-btn rounded-full cursor-pointer touch-none"
					bind:this={volumeBar}
				...
				>
					<div
						class="h-full bg-\[var\(--music-player-primary\)\] rounded-full transition-all"
		...
		{#if showPlaylist}
			...
			>
				<div
					class="playlist-header flex items-center justify-between p-4 mp-border-divider"
				>
					<h3 class="text-lg font-semibold mp-text-primary">
				...
				>
					{#each playlist as song, index}
						<div
							class="playlist-item flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors"
						...
						>
							...
							>
								{#if index === currentIndex && isPlaying}
									<Icon
										...
										class="text-\[var\(--music-player-primary\)\] animate-pulse"
									/>
								{:else if index === currentIndex}
									<Icon
										icon="material-symbols:pause"
										class="text-\[var\(--music-player-primary\)\]"
									/>
								{:else}
									<span class="text-sm mp-text-meta"
										>{index + 1}</span
									>
								{/if}
							</div>
							<div
								class="w-10 h-10 rounded-lg overflow-hidden mp-bg-btn shrink-0"
							>
								<img
							...
							<div class="flex-1 min-w-0">
								<div
									class="font-medium truncate mp-text-primary"
								>
									{song.title}
								</div>
